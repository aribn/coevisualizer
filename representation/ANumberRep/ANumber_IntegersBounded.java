...
public class ANumber_IntegersBounded implements ANumber, Test, Candidate {

    private int[] n;
    private final int RESOLUTION = 100;


    public ANumber_IntegersBounded() {
        super();
    }
	...
	// BOUND THE MUTATIONS TO WITHIN [0,100]
	
    public PopulationMember getMutation (Random r, double mRate, int mBias, double mSize) {
		
        int[] mutation = new int[n.length];
        int[] change = new int[n.length];
		
        for (int i=0; i<mutation.length; i++) {
            mutation[i] = n[i];
            if(r.nextDouble() < mRate) {
                double bias = (double) (mBias * 2 * (RESOLUTION/2 - mutation[i])/RESOLUTION);
                change[i] = (int) Math.round(r.nextGaussian()*mSize + bias);
                if ((n[i] + change[i]) > 100)    mutation[i] = n[i] - (int) Math.abs(change[i]);
                else if ((n[i] + change[i]) < 0) mutation[i] = n[i] + (int) Math.abs(change[i]);
                else 				 mutation[i] = n[i] + change[i];
                //System.out.println("n[i] was "+n[i] + ", m[i] is "+mutation[i]);
            }
        }
		
        ANumber_IntegersBounded ani = new ANumber_IntegersBounded();
